...
//   srand(time(NULL));
  ...
  auto function_kim2 = [&fpm](const Eigen::Ref<const Eigen::VectorXd> &x, const Eigen::Ref<const Eigen::VectorXd> &q, Eigen::Ref<Eigen::VectorXd> out) {
    ...
    a << 0.0, 0.0, 0.0, 0.0825, -0.0825, 0.0, 0.088;
    d << 0.333, 0.0, 0.316, 0.0, 0.384, 0.0, 0.0;
    for (int i=0; i<7; i++){T_0e = T_0e * transformDH(a(i) + x(0 + i*3) , d(i) + x(1 + i*3) , M_PI_2 * alpha(i), q(i)+q_offset(i));}
  ...
  auto jacobian_dh = [function_kim2](const Eigen::Ref<const Eigen::VectorXd> &x, const Eigen::Ref<const Eigen::VectorXd> &q, Eigen::Ref<Eigen::MatrixXd> out) {
    Eigen::VectorXd y1 = x;
    Eigen::VectorXd y2 = x;
    Eigen::VectorXd t1(3);
    Eigen::VectorXd t2(3);
    ...
    for (std::size_t j = 0; j < 21; j++)
    {
      ...
      y1[j] += h;
      y2[j] -= h;
      function_kim2(y1, q, t1);
      function_kim2(y2, q, t2);
      const Eigen::VectorXd m1 = (t1 - t2) / (y1[j] - y2[j]);
      y1[j] += h;
      y2[j] -= h;
      function_kim2(y1, q, t1);
  ...
  auto jacobian_kim2 = [&fpm](const Eigen::Ref<const Eigen::VectorXd> &x, const Eigen::Ref<const Eigen::VectorXd> &q, Eigen::Ref<Eigen::MatrixXd> out)
  {
    ...
    for (int i=0 ; i<7; i++){dh.row(i).head<3>() = x.segment<3>(i*3);}
    Eigen::Ref<Eigen::VectorXd> q_offset = dh.col(2);
    const Eigen::Ref<const Eigen::VectorXd> &aaa = dh.col(0);
    const Eigen::Ref<const Eigen::VectorXd> &ddd = dh.col(1);
    fpm.initModel(dh);
    T_0e = fpm.getTransform(q+q_offset);

    for (int i=0; i<7; i++)
    {
      // T_0e_test = T_0e_test * transformDH(a(i) + x(0 + i*3), d(i) + x(1 + i*3) , M_PI_2 * alpha(i), q(i)+q_offset(i));
      T_0e_test = T_0e_test * transformDH(a(i) + aaa(i), d(i) + ddd(i) , M_PI_2 * alpha(i), q(i)+q_offset(i));
      // T_0e_test = T_0e_test * transformDH(a(i), d(i), M_PI_2 * alpha(i), q(i)+q_offset(i));
    }
    T_0e_test = T_0e_test * transformDH(0.0, 0.107, 0.0, 0.0);

    // std::cout << "testing T_0e\n" << "T_0e from fpm:\n" << T_0e.matrix()<< std::endl;
    std::cout <<"\nT_0e from my function:\n"<<T_0e_test.matrix()<<std::endl;
    for (int i=0; i<7; i++)
    {
      x_0i_1 = x_0i;
      y_0i_1 = y_0i;
      ...
      if(alpha(i)==0)
      {
        std::cout<<"alpha == 0 for joint"<<i<<std::endl;
        x_0i = sin(q(i)+q_offset(i)) * y_0i_1 + cos(q(i)+q_offset(i)) * x_0i_1;
        y_0i = cos(q(i)+q_offset(i)) * y_0i_1 - sin(q(i)+q_offset(i)) * x_0i_1;
        z_0i = z_0i_1;
      }
      else
      {
        std::cout<<"alpha /= 0 for joint"<<i<<std::endl;
        x_0i = alpha(i) * sin(q(i)+q_offset(i)) * z_0i_1 + cos(q(i)+q_offset(i)) * x_0i_1;
        y_0i = alpha(i) * cos(q(i)+q_offset(i)) * z_0i_1 - sin(q(i)+q_offset(i)) * x_0i_1;
        z_0i = -1.0 * alpha(i) * y_0i_1;
      }
      // if(alpha(i)==0)
      // {
      //   x_0i = sin(q(i)+q_offset(i)) * y_0i_1 + cos(q(i)+q_offset(i)) * x_0i_1;
      ...
      //   z_0i = alpha(i) * y_0i_1;
      // }
      T_0i = T_0i * transformDH(a(i) + x(0 + i*3), d(i) + x(1 + i*3), M_PI_2 * alpha(i), q(i)+q_offset(i));
      R_0i = T_0i.linear();
  ...
  for (int i=0; i<q_input_1.size(); ++i)
  {
    // true?
    // function_kim2(x, q_input_1[i], )
    // Eigen::VectorXd x(21);
    // function_kim2()
  }
