...
//         z_0i = alpha(i) * y_0i_1;
//       }
//       T_0i = T_0i * transformKim(a(i) + x(0 + i*3), d(i) + x(1 + i*3), M_PI_2 * alpha(i), q(i)+x(2 + i*3));
//       R_0i = T_0i.linear();
...
//   srand(time(NULL));
  ...
  auto function_kim2 = [&fpm](const Eigen::Ref<const Eigen::VectorXd> &x, const Eigen::Ref<const Eigen::VectorXd> &q, Eigen::Ref<Eigen::VectorXd> out) {
    ...
    T_0e.setIdentity();
    Eigen::VectorXd alpha(7), a(7), d(7);
    alpha << 0.0, -1.0*M_PI_2, M_PI_2, M_PI_2, -1.0*M_PI_2, M_PI_2, M_PI_2;
    a << 0.0, 0.0, 0.0, 0.0825, -0.0825, 0.0, 0.088;
    d << 0.333, 0.0, 0.316, 0.0, 0.384, 0.0, 0.0;
    for (int i=0; i<7; i++)
    {
      T_0e = T_0e * transformKim(a(i) + a_offset(i), d(i) + d_offset(i) ,  alpha(i)+alpha_offset(i), q(i)+q_offset(i));
    }
  ...
  auto jacobian_kim2 = [&fpm](const Eigen::Ref<const Eigen::VectorXd> &x, const Eigen::Ref<const Eigen::VectorXd> &q, Eigen::Ref<Eigen::MatrixXd> out)
  {
    ...
    for (int i=0; i<7; i++)
    {
      T_0e_test = T_0e_test * transformKim(a(i) + a_offset(i), d(i) + d_offset(i) ,  alpha(i)+alpha_offset(i), q(i)+q_offset(i));
    }
    ...
    for (int i=0; i<7; i++)
    {
      ...
      x_0i = cos(q(i)+q_offset(i)) * x_0i_1               + sin(q(i)+q_offset(i)) * cos(alpha(i)+alpha_offset(i)) * y_0i_1 + sin(q(i)+q_offset(i)) * sin(alpha(i)+alpha_offset(i)) * z_0i_1;
      y_0i = -1.0*sin(q(i)+q_offset(i)) * x_0i_1          + cos(q(i)+q_offset(i)) * cos(alpha(i)+alpha_offset(i)) * y_0i_1 + cos(q(i)+q_offset(i)) * sin(alpha(i)+alpha_offset(i)) * z_0i_1;
      z_0i = -1.0*sin(alpha(i)+alpha_offset(i)) * y_0i_1  + cos(alpha(i)+alpha_offset(i)) * z_0i_1;

      T_0i = T_0i * transformKim(a(i) + x(0 + i*3), d(i) + x(1 + i*3), alpha(i), q(i)+q_offset(i));
      R_0i = T_0i.linear();
      p_ie = (T_0i.inverse() * T_0e).translation();
      jacob_k.col(0 + i*3) += x_0i_1;
      jacob_k.col(1 + i*3) += z_0i;
